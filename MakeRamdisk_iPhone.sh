...
download_File(){
	...
	then
		...
		then
            ...
            "$dirBIN"/pzb -g "$(awk "/""${replace}""/{x=1}x&&/$1[.]/{print;exit}" BuildManifest.plist | grep '<string>' |cut -d\> -f2 |cut -d\< -f1)" "$URL"
	fi
	cd ..
}


curl -s "https://www.theiphonewiki.com/wiki/Firmware_Keys/$ver.x" | grep "$dos" | grep "$uno" | awk '{printf $2" "}' | awk '{printf $1}' | sed 's/href="/"https:\/\/www.theiphonewiki.com/g' &>.url
